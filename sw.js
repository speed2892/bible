...
/* 알림 시간 (기본 08:00, 앱에서 메시지로 갱신) */
let alarmHour = 8, alarmMin = 0;
...
function msUntilNextAlarm() {
  const now  = new Date();
  const next = new Date(now.getFullYear(), now.getMonth(), now.getDate(), alarmHour, alarmMin, 0, 0);
...
self.addEventListener("message", e => {
  if (e.data && e.data.type === "SCHEDULE_ALARM") {
    if (e.data.alarmTime) setAlarmTime(e.data.alarmTime);
    scheduleNext();
