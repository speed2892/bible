/* ══════════════════════════════════════
   ...
   setInterval 방식: 매분 시간 체크 → 알림 발송
   앱이 닫혀도 SW가 살아있으면 동작
══════════════════════════════════════ */

const CACHE_NAME = "bible-2026-v2";
...
function getTodayReading(date) {
  ...
  const text = groups.map(g =>
    g.startNum === g.endNum
      ? `${g.name} ${g.startNum}장`
      : `${g.name} ${g.startNum}~${g.endNum}장`
  ).join(" / ");
  return {text, count: chapters.length, day: di + 1};
}

/* ══════════════════════════════════════
   알림 설정 — IndexedDB에서 읽기
══════════════════════════════════════ */
let alarmOn   = false;
let alarmHour = 8;
let alarmMin  = 0;
let lastNotifiedDate = ""; // "YYYY-MM-DD" — 하루 한 번만 발송

/* IndexedDB에서 알림 설정 로드 */
async function loadAlarmSettings() {
  try {
    const db = await openSettingsDB();
    const on   = await dbGet(db, "alarm_on");
    const time = await dbGet(db, "alarm_time");
    alarmOn = (on === "1");
    if (time) {
      const [h, m] = time.split(":").map(Number);
      if (!isNaN(h) && !isNaN(m)) { alarmHour = h; alarmMin = m; }
    }
  } catch(e) { /* 실패 시 기본값 사용 */ }
}
...
function openSettingsDB() {
  return new Promise((res, rej) => {
    const req = indexedDB.open("bibleDB", 1);
    req.onupgradeneeded = e => e.target.result.createObjectStore("checks");
...
function dbGet(db, key) {
  return new Promise((res, rej) => {
    const req = db.transaction("checks","readonly").objectStore("checks").get(key);
...
function dbSet(db, key, val) {
  return new Promise((res, rej) => {
    const req = db.transaction("checks","readwrite").objectStore("checks").put(val, key);
    req.onsuccess = () => res();
    req.onerror   = e => rej(e);
  });
}
...
function startIntervalCheck() {
  setInterval(async () => {
    ...
    // 설정한 시각과 일치하고 오늘 아직 알림 안 보냈으면 발송
    if (h === alarmHour && m === alarmMin && lastNotifiedDate !== today) {
...
self.addEventListener("message", async e => {
  ...
  if (e.data.type === "SCHEDULE_ALARM") {
    try {
      const db = await openSettingsDB();
      await dbSet(db, "alarm_on", "1");
      if (e.data.alarmTime) {
        await dbSet(db, "alarm_time", e.data.alarmTime);
        const [h, m] = e.data.alarmTime.split(":").map(Number);
        if (!isNaN(h) && !isNaN(m)) { alarmHour = h; alarmMin = m; }
      }
      alarmOn = true;
    } catch(e) {}
...
/* SW 활성화 시 인터벌 시작 */
loadAlarmSettings().then(() => startIntervalCheck());
